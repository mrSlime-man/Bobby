...
from src.llm.llm_manager import (
    ...
    OllamaModel,
    OpenAIModel,
    build_education_status_context,
    has_unsupported_student_inference,
    normalize_optional_proxy,
)


def test_template_proxy_is_treated_as_unconfigured():
    assert (
        normalize_optional_proxy("[Your LLM Proxy in format http(s)://login:password@host:port]")
        is None
    )
    assert normalize_optional_proxy("http://proxy.example.com:8080") == (
        "http://proxy.example.com:8080"
    )


def test_completion_year_does_not_imply_current_student_status():
    context = build_education_status_context(
        {
            "education_details": [
    ...
    assert "Credential: High School Diploma" in context
    assert "graduation/completion year: 2026" in context
    assert "Current enrollment/student status: not stated" in context
    assert has_unsupported_student_inference(
        "Score: 40\nReasoning: The candidate is currently a high school student.", context
    )
...
class TestLLMLogger:
    ...
    def test_llm_logger_log_request_includes_timing_and_context(self, mock_append_yaml):
        ...
        assert payload["job_url"] == "[REDACTED]"
        assert payload["job_title"] == "[REDACTED]"
        assert payload["company_name"] == "[REDACTED]"
        assert payload["prompts"] == {"prompt_1": "<prompt omitted>"}
        assert payload["parsed_reply"] == "<response omitted>"
...
class TestGPTAnswerer:
    ...
    @patch("src.llm.llm_manager.AIAdapter")
    @patch("src.llm.llm_manager.LoggerChatModel")
    def test_gpt_answerer_uses_source_description_when_optional_summary_fails(
        self,
        mock_logger_chat,
        mock_ai_adapter,
        mock_transform,
        ...
        """A summary-provider error must not abort an otherwise admissible job."""
        mock_transform.return_value = "Original vacancy description"
        summary_chain = MagicMock()
        summary_chain.invoke.side_effect = TimeoutError("provider timeout")

        answerer = GPTAnswerer(mock_api_key, mock_llm_proxy)
        answerer.chains["summarize_job_description"] = summary_chain

        answerer.set_job(mock_job)

        assert answerer.job_readable == "Original vacancy description"
        summary_chain.invoke.assert_called_once()
    ...
    @patch("src.llm.llm_manager.AIAdapter")
    @patch("src.llm.llm_manager.LoggerChatModel")
    def test_suitability_log_uses_allowlisted_score_and_category_only(
        self,
        mock_logger_chat,
        mock_ai_adapter,
        mock_api_key,
        ...
        private_reason = (
            "Needs 9 years experience. person@example.com +1 555-123-4567, "
            "12 Private Street, degree details, salary $123456. " + "private reasoning " * 30
        )
        chain = MagicMock()
        chain.invoke.return_value = f"Score: 25\nReasoning: {private_reason}"
        answerer = GPTAnswerer(mock_api_key, mock_llm_proxy)
        answerer.chains["job_is_interesting"] = chain
        answerer.set_resume(mock_resume_structured, mock_resume_readable)
        answerer.set_job(mock_job, is_test=True)
        answerer.search_parameters = "Remote: True"

        with patch("src.llm.llm_manager.logger") as safe_logger:
            result = answerer.job_is_interesting(mock_job)

        assert result[0] is False
        logged = " ".join(str(call) for call in safe_logger.method_calls)
        assert "SUITABILITY_SKIP | score=25 | reason=experience_mismatch" in logged
        assert "person@example.com" not in logged
        assert "555-123-4567" not in logged
        assert "Private Street" not in logged
    ...
    @patch("src.llm.llm_manager.AIAdapter")
    @patch("src.llm.llm_manager.LoggerChatModel")
    def test_job_scoring_retries_unsupported_current_student_inference(
        self,
        mock_logger_chat,
        mock_ai_adapter,
        mock_api_key,
        ...
        structured = {
            "personal_information": {"gender": "male", "phone_code": "+1"},
            "education_details": [
                {"education_level": "High School Diploma", "year_of_completion": 2026}
            ],
        }
        ...
        chain.invoke.side_effect = [
            "Score: 40\nReasoning: The candidate is currently a high school student.",
            "Score: 80\nReasoning: The diploma requirement is satisfied.",
        ]
        answerer = GPTAnswerer(mock_api_key, mock_llm_proxy)
        answerer.chains["job_is_interesting"] = chain
        answerer.set_resume(structured, mock_resume_readable)
        answerer.search_parameters = "Remote: True"
        ...
        assert is_interesting is True
        assert score == "80"
        assert "student" not in reasoning.casefold()
        assert chain.invoke.call_count == 2
        second_context = chain.invoke.call_args_list[1].args[0]["education_status_context"]
        assert "not evidence that the candidate is currently a student" in second_context

    @patch("src.llm.llm_manager.open_circuit")
    @patch("src.llm.llm_manager.circuit_is_open", return_value=False)
    @patch("src.llm.llm_manager.JOB_IS_INTERESTING_THRESH", 70)
    @patch("src.llm.llm_manager.AIAdapter")
    @patch("src.llm.llm_manager.LoggerChatModel")
    def test_job_scoring_falls_back_to_openai_after_primary_failure(
        self,
        mock_logger_chat,
        mock_ai_adapter,
        _mock_circuit,
        ...
        answerer.search_parameters = "Remote: True"
        primary = MagicMock()
        primary.invoke.side_effect = RuntimeError("503 UNAVAILABLE")
        fallback = MagicMock()
        fallback.invoke.return_value = "Score: 85\nReasoning: Fallback provider match"
        answerer.chains["job_is_interesting"] = primary
        answerer._suitability_provider_order = ("gemini", "openai")
        answerer._suitability_chain_for_provider = MagicMock(
            side_effect=lambda provider: primary if provider == "gemini" else fallback
        )

        result = answerer.job_is_interesting(mock_job)

        assert result == (True, "85", "Fallback provider match")
