...
sys.modules["browser_use.agent.views"] = MagicMock()

from main import (
    ConfigError,
    ConfigValidator,
    browser_recovery_allowed,
    check_pause,
    create_and_run_bot,
    main,
)
...
class TestGracefulDrainEntry:
    @pytest.mark.asyncio
    async def test_dashboard_stop_enters_drain_without_aborting_active_callback(self):
        runtime_controller.shutdown_requested.clear()
        runtime_controller.begin_run()
        try:
            with (
                patch.dict(os.environ, {"DASHBOARD_RUN_ID": "run-test"}),
                patch(
                    "main.get_control_state",
                    return_value={"stop_requested": True, "pause_requested": False},
                ),
            ):
                await check_pause()

            assert runtime_controller.shutdown_state == ShutdownState.DRAINING
            assert runtime_controller.is_shutdown_requested()
        finally:
            runtime_controller.shutdown_requested.clear()
            runtime_controller.begin_run()

    @pytest.mark.asyncio
    async def test_pending_shutdown_refuses_to_start_new_bot_run(self):
        runtime_controller.shutdown_requested.clear()
        runtime_controller.begin_run()
        runtime_controller.request_shutdown("startup-test")
        try:
            with (
                patch("main.create_playwright_browser", new_callable=AsyncMock) as create_browser,
                patch("main.emit_event") as emit,
            ):
                result = await create_and_run_bot({}, {}, "", {})
        finally:
            runtime_controller.shutdown_requested.clear()
            runtime_controller.begin_run()
        ...
        assert emit.call_args.args[0] == "run_stopped"

    def test_browser_failure_after_shutdown_cannot_restart(self):
        runtime_controller.shutdown_requested.clear()
        runtime_controller.begin_run()
        runtime_controller.request_shutdown("SIGINT")
        try:
            assert browser_recovery_allowed(True, False) is False
        finally:
            runtime_controller.shutdown_requested.clear()
            runtime_controller.begin_run()

    @pytest.mark.asyncio
    async def test_shutdown_after_browser_creation_prevents_login_and_filters(self):
        runtime_controller.shutdown_requested.clear()
        runtime_controller.begin_run()

        async def create_then_shutdown():
            runtime_controller.request_shutdown("startup-race")
            return None, MagicMock(), MagicMock()

        try:
            with (
                patch("main.create_playwright_browser", side_effect=create_then_shutdown),
                patch("main.Authenticator") as authenticator,
                patch("main.save_browser_session", new_callable=AsyncMock),
                patch("main.stop_tracing", new_callable=AsyncMock),
            ):
                result = await create_and_run_bot({}, {}, "", {})
        finally:
            runtime_controller.shutdown_requested.clear()
...
class TestMain:
    ...
    def test_main_success(self):
        ...
        with (
            ...
            patch("main.validate_candidate_identity", return_value="John Doe"),
