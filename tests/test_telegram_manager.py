...
class TestTelegramReportSender:
    ...
    @pytest.mark.asyncio
    async def test_unverified_after_submit_never_renders_as_telegram_success(
        self, mock_env, mock_config
    ):
        with patch("src.telegram.telegram_manager.Bot") as mock_bot_class:
            mock_bot = AsyncMock()
            mock_bot_class.return_value = mock_bot
            ...
            await sender.send_job_event(
                job_title="Support Engineer",
                company_name="Example",
                url="https://example.com/job",
                result="Error",
                reason="UNVERIFIED_AFTER_SUBMIT: no independent receipt",
                final_status="UNVERIFIED_AFTER_SUBMIT",
            )

        message = mock_bot.send_message.await_args.kwargs["text"]
        assert "⚠️ UNVERIFIED AFTER SUBMIT" in message
        assert "✅ SUBMITTED" not in message
        assert "✅ APPLIED" not in message
        assert "Application submitted" not in message

    @pytest.mark.asyncio
    async def test_verified_submission_renders_one_submitted_status(
        self, mock_env, mock_config
    ):
        with patch("src.telegram.telegram_manager.Bot") as mock_bot_class:
            mock_bot = AsyncMock()
        ...
        message = mock_bot.send_message.await_args.kwargs["text"]
        assert message.count("✅ SUBMITTED") == 1
        assert "Independent submission confirmation received" in message

    @pytest.mark.asyncio
    async def test_suitability_event_does_not_publish_candidate_details(
        self, mock_env, mock_config
    ):
        private_reason = (
            "The candidate person@example.com at +1 555-123-4567 and "
            "12 Private Street lacks 8 years experience, has degree notes, "
            "and requested salary $123456. " + "long private reasoning " * 30
        )
        with patch("src.telegram.telegram_manager.Bot") as mock_bot_class:
            mock_bot = AsyncMock()
            mock_bot_class.return_value = mock_bot
        ...
        message = mock_bot.send_message.await_args.kwargs["text"]
        assert "Support Engineer" in message
        assert "Example Company" in message
        assert "Suitability score 25; category: experience_mismatch" in message
        for private_text in (
            "person@example.com",
            "555-123-4567",
            "Private Street",
    ...
    @pytest.mark.asyncio
    async def test_compact_report_uses_authoritative_cycle_stats(self, mock_env, mock_config):
        with patch("src.telegram.telegram_manager.Bot") as mock_bot_class:
            mock_bot_class.return_value = AsyncMock()
            sender = TelegramReportSender()
            sender._send_chunked_messages = AsyncMock()
            await sender.send_compact_run_report(
                applied=0,
                skipped=0,
                failed=0,
                viewed=0,
        ...
        message = sender._send_chunked_messages.call_args.args[0]
        assert "Found: 55" in message
        assert "Encountered: 55" in message
        assert "Attempted: 0" in message
        assert "Submitted: 0" in message
        assert "Skipped: 55" in message
    @pytest.mark.asyncio
...
def test_redact_sensitive_text_hides_codes_tokens_and_passwords():
    from src.telegram.telegram_manager import redact_sensitive_text

    value = redact_sensitive_text(
        "OTP: 123456 password=hunter2 "
        "https://example.invalid/verify?token=abc123&keep=yes"
    )

    assert "123456" not in value
    assert "hunter2" not in value
    assert "abc123" not in value
    assert "keep=yes" in value


def test_redact_sensitive_text_hides_plain_job_urls():
    from src.telegram.telegram_manager import redact_sensitive_text

    value = redact_sensitive_text("Open https://jobs.example.invalid/view/123")

    assert "jobs.example.invalid" not in value
