...
from src.job_manager.job_manager import classify_final_application_result
...
def test_explicit_security_challenge_remains_needs_human():
    result = classify_final_application_result(
        ("Skip", "NEEDS_HUMAN: CAPTCHA requires manual verification")
    )
...
def isolate_process_run_aggregate():
    with patch(
        "src.job_manager.job_manager.runtime_controller", RuntimeController()
    ):
        yield
...
class TestAuthoritativeResultCounters:
    @staticmethod
    def _initialize_result_state(job_applier):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("Success", "", 1, 0, 0, 0, 0, "SUBMITTED"),
            (
                "Success",
                "UNVERIFIED_AFTER_SUBMIT: no receipt",
                0,
                1,
                0,
                1,
            ...
            (
                "Error",
                "TECHNICAL_FAILURE: browser failed",
                0,
                1,
                0,
                0,
            ...
            (
                "Cancelled",
                "CANCELLED_BY_SHUTDOWN: pre-submit",
                0,
                0,
                1,
                0,
    ...
    async def test_only_verified_success_increments_sent_counter(
        self,
        job_applier,
        result,
        reason,
        ...
        with (
            patch("src.job_manager.job_manager.TelegramReportSender") as telegram,
            patch("src.job_manager.job_manager.emit_event") as emit,
            patch.object(job_applier, "_save_company") as save_company,
            patch.object(job_applier, "_write_the_last_search_time"),
        ):
            telegram.return_value.send_job_event = AsyncMock()
            await job_applier._handle_apply_result((result, reason), job)

        assert job_applier.cycle_success_num == submitted
        assert job_applier.cycle_failed_num == failed
        assert job_applier.cycle_cancelled_num == cancelled
        assert job_applier.cycle_unverified_num == unverified
        assert job_applier.cycle_technical_failure_num == technical
        sent_result = telegram.return_value.send_job_event.await_args.kwargs["result"]
        assert sent_result == ("Error" if classification == "UNVERIFIED_AFTER_SUBMIT" else result)
        assert telegram.return_value.send_job_event.await_args.kwargs["final_status"] == classification
        emit.assert_called_once()
        assert emit.call_args.kwargs["classification"] == classification
        if result == "Cancelled":
            save_company.assert_not_called()

    @pytest.mark.asyncio
    async def test_verified_submission_is_finalized_then_published_once(self, job_applier):
        self._initialize_result_state(job_applier)
        order = []
        job = Job(
            job_title="Role",
            company_name="Company",
            url="https://www.linkedin.com/jobs/view/verified",
        )
        telegram_sender = MagicMock()
        telegram_sender.send_job_event = AsyncMock(
            side_effect=lambda **_kwargs: order.append("telegram")
        )
        ...
        with (
            patch.object(
                job_applier,
                "_save_company",
                side_effect=lambda *_args, **_kwargs: order.append("persist_result"),
            ),
            patch.object(
                job_applier,
                "_write_the_last_search_time",
                side_effect=lambda: order.append("persist_counters"),
            ),
            patch(
                "src.job_manager.job_manager.emit_event",
                side_effect=lambda *_args, **_kwargs: order.append("event"),
            ) as emit,
            patch(
                "src.job_manager.job_manager.TelegramReportSender",
                return_value=telegram_sender,
            ),
        ):
            await job_applier._handle_apply_result(("Success", ""), job)

        assert order == ["persist_result", "persist_counters", "event", "telegram"]
        assert job_applier.cycle_success_num == 1
        emit.assert_called_once()
        assert emit.call_args.args[:2] == ("job_result", "Job result: SUBMITTED")
        assert emit.call_args.kwargs["classification"] == "SUBMITTED"
        telegram_sender.send_job_event.assert_awaited_once()
        assert telegram_sender.send_job_event.await_args.kwargs["final_status"] == "SUBMITTED"

    @pytest.mark.asyncio
    async def test_unverified_submit_publishes_only_unverified_never_success(
        self, job_applier
    ):
        self._initialize_result_state(job_applier)
        job = Job(
            job_title="Role",
            company_name="Company",
            url="https://www.linkedin.com/jobs/view/unverified",
        )
        telegram_sender = MagicMock()
        telegram_sender.send_job_event = AsyncMock()
        ...
        with (
            ...
            await job_applier._handle_apply_result(
                ("Success", "UNVERIFIED_AFTER_SUBMIT: no independent receipt"),
                job,
            )

        assert job_applier.cycle_success_num == 0
        assert job_applier.cycle_unverified_num == 1
        save_company.assert_called_once_with(
            job,
            ("Error", "UNVERIFIED_AFTER_SUBMIT: no independent receipt"),
            {"url": job.url},
            evaluation={},
        )
        emit.assert_called_once()
        assert emit.call_args.kwargs["result"] == "error"
        assert emit.call_args.kwargs["classification"] == "UNVERIFIED_AFTER_SUBMIT"
        telegram_sender.send_job_event.assert_awaited_once()
        telegram_call = telegram_sender.send_job_event.await_args.kwargs
        assert telegram_call["result"] == "Error"
        assert telegram_call["final_status"] == "UNVERIFIED_AFTER_SUBMIT"

    @pytest.mark.asyncio
    async def test_persistence_failure_emits_no_event_or_telegram_success(self, job_applier):
        self._initialize_result_state(job_applier)
        job = Job(
            job_title="Role",
            company_name="Company",
            url="https://www.linkedin.com/jobs/view/persist-failure",
        )

        with (
            patch.object(job_applier, "_save_company", side_effect=RuntimeError("disk full")),
            patch("src.job_manager.job_manager.emit_event") as emit,
            patch("src.job_manager.job_manager.TelegramReportSender") as telegram,
        ):
...
class TestSendReport:
    ...
    @pytest.mark.asyncio
    async def test_send_report_includes_cycle_even_when_result_has_error(self, job_applier):
        job_applier.previous_apply_number = 0
        job_applier.success_applies_num = 5
        ...
        with (
            patch("src.job_manager.job_manager.TEST_MODE", False),
            patch("src.job_manager.job_manager.COLLECT_INFO_MODE", False),
            patch("src.job_manager.job_manager.TelegramReportSender") as mock_bot,
        ):
            mock_bot.return_value.send_compact_run_report = AsyncMock()
            await job_applier.send_report("Error")

        mock_bot.return_value.send_compact_run_report.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_send_report_includes_all_encountered_cycle(self, job_applier):
    ...
    async def test_send_report_sends_when_conditions_met(self, job_applier):
        ...
        mock_bot_instance.send_compact_run_report = AsyncMock()
        ...
        mock_bot_instance.send_compact_run_report.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_partial_report_uses_restart_proof_run_aggregate(self, job_applier):
        controller = RuntimeController()
        controller.start_process_run("run-aggregate")
        controller.record_discovered_job("greystar", encountered=False)
        controller.record_attempt("external")
        controller.record_terminal_outcome("greystar", "TECHNICAL_FAILURE")
        controller.set_shutdown_state(ShutdownState.CLEANUP)
        controller.set_shutdown_state(ShutdownState.DONE)
        assert controller.begin_run()
        controller.record_discovered_job("leidos", encountered=False)
        controller.record_terminal_outcome("leidos", "SKIPPED")
        controller.record_discovered_job("collabera", encountered=True)
        controller.record_discovered_job("maritime", encountered=False)
        controller.request_shutdown("SIGINT")
        controller.record_terminal_outcome("maritime", "CANCELLED")
        job_applier.cycle_partial = True

        mock_sender = MagicMock()
        mock_sender.send_compact_run_report = AsyncMock()
        with (
            patch("src.job_manager.job_manager.TEST_MODE", False),
            patch("src.job_manager.job_manager.COLLECT_INFO_MODE", False),
            patch("src.job_manager.job_manager.runtime_controller", controller),
            patch(
                "src.job_manager.job_manager.TelegramReportSender",
            ...
            await job_applier.send_report("Shutdown")

        stats = mock_sender.send_compact_run_report.await_args.kwargs["cycle_stats"]
