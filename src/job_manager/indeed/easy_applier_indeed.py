    ...
    async def _handle_resume_selection(self, job: Job) -> None:
        ...
        try:
            ...
            if self.ready_made_resume_path is not None:
                ...
                logger.info("Uploaded configured ready-made resume")
            else:
        ...
        try:
            ...
            if address:
                ...
                if field:
                    ...
                    logger.debug("Filled street address from structured profile")
        ...
        try:
            ...
            if title_input and position:
                ...
                if not current_value:
                    ...
                    logger.debug("Filled relevant experience job title from structured profile")
                else:
                    logger.debug("Relevant experience job title was already filled")
            ...
            if company_input and company:
                ...
                if not current_value:
                    ...
                    logger.debug("Filled relevant experience company from structured profile")
                else:
                    logger.debug("Relevant experience company was already filled")
        ...
        try:
            ...
            if work_cards:
                ...
                if card_container:
                    ...
                    logger.info("Selected relevant experience card")
            else:
                logger.info(
    ...
    async def _find_and_handle_date_question(self, section: Any) -> bool:
        ...
        try:
            ...
            if existing_answer:
                ...
                logger.debug(
                    f"Using cached date answer for '{redact_text(question_text)}' "
                    "(value omitted)"
                )
            else:
            ...
            logger.debug(
                f"Filled date field '{redact_text(question_text)}' (value omitted)"
            )
        except NoInfoException:
            raise
        except Exception as e:
    ...
    async def _find_and_handle_textbox_question(self, section: Any) -> bool:
        ...
        try:
            ...
            if existing_answer:
                answer = existing_answer
                logger.debug(
                    f"Using cached answer for '{redact_text(question_text)}' "
                    "(value omitted)"
                )
            elif is_numeric:
    ...
    async def _find_and_handle_radio_question(self, section: Any) -> bool:
        ...
        try:
            ...
            if existing_answer:
                answer = existing_answer
                logger.debug(
                    f"Using cached radio answer for '{redact_text(question_text)}' "
                    "(value omitted)"
                )
            else:
    ...
    async def _find_and_handle_dropdown_question(self, section: Any) -> bool:
        ...
        try:
            ...
            if existing_answer:
                answer = existing_answer
                logger.debug(
                    f"Using cached dropdown answer for '{redact_text(question_text)}' "
                    "(value omitted)"
                )
            else:
                answer = self.gpt_answerer.select_one_answer_from_options(
    ...
    async def _fill_textbox_question_errors(self) -> bool:
        ...
        for element, question_text, error_text in results:
            ...
            try:
                ...
                logger.debug(
                    f"Re-filled '{redact_text(question_text)}' (value omitted)"
                )
            except NoInfoException:
                raise
            except Exception as e:
