...
class BotFacade:
    ...
    async def set_search_parameters(self, parameters: Dict[str, Any]) -> None:
        ...
        verified = await self.search_component.set_search_params()
        if verified is not True or getattr(self.search_component, "filters_verified", False) is not True:
            logger.error("LinkedIn profile filter verification failed; job intake is blocked")
            raise RuntimeError("LINKEDIN_FILTER_VERIFICATION_FAILED")
