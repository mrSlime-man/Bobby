...
def _event_has_job_identity(payload: Dict[str, Any]) -> bool:
    """Keep run-job views free of aggregate runtime telemetry placeholders."""
...
def _build_run_jobs(run_id: str) -> List[Dict[str, Any]]:
    ...
    for event in events:
        ...
        if not isinstance(payload, dict) or not _event_has_job_identity(payload):
            continue
        url = (
            payload.get("url")
